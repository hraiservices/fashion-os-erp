...
function validateRows(table: ParsedTable, mapping: ImportMapping): ImportRowResult[] {
  ...
  return table.rows.map((row, rowIndex) => {
    ...
    const total = totalRaw ? parseFloat(totalRaw) : NaN;
    ...
    else if (!customerName) error = "Missing customer name";
    else if (!garmentType) error = "Missing garment type";
    else if (!totalRaw || Number.isNaN(total) || total < 0) error = "Invalid total";
    else if (Number.isNaN(advance) || advance < 0) error = "Invalid advance";
    ...
    <div className="space-y-5">
      {!table ? (
        <div className="rounded-xl border border-dashed p-8 text-center">
          ...
          <p className="mb-3 text-sm text-muted-foreground">Upload a CSV or TSV file — one row per order.</p>
                  ...
                  <TableBody>
                    {results.map((r) => (
                      <TableRow key={r.rowIndex}>
                        ...
                        <TableCell>{r.garmentType}</TableCell>
