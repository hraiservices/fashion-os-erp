      ...
      {lines.length === 0 ? (
        ...
        lines.map((line) => (
          <div key={line.key} className="flex items-center gap-2">
            ...
            />
            <Input type="number" inputMode="decimal" min={0} step="0.001" placeholder="Qty" className="w-24 h-10" value={line.qty} onChange={(e) => updateLine(line.key, { qty: e.target.value })} />
            <Input type="number" inputMode="decimal" min={0} step="0.01" placeholder="Cost/unit" className="w-28 h-10" value={line.unitCost} onChange={(e) => updateLine(line.key, { unitCost: e.target.value })} />
