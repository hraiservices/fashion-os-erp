...
function SelectTrigger({
  ...
  return (
    <SelectPrimitive.Trigger
      ...
      data-size={size}
      className={cn(
        // Height is a plain (non-variant) utility here — not data-[size=...]:h-N — specifically
        // so it has normal specificity and a caller's className can actually override it via
        // twMerge. A data-attribute-scoped height utility has higher CSS specificity than a
        // plain className override and silently wins regardless of source order, which is why
        // every consumer's className="h-10" was previously being ignored (still rendering h-8).
        ...
        size === "sm" ? "h-11 sm:h-7" : "h-11 sm:h-8",
        className
      )}
      {...props}
    >
