...
function validateRows(table: ParsedTable, mapping: ImportMapping, existingCustomers: Customer[]): ImportRowResult[] {
  ...
  return table.rows.map((row, rowIndex) => {
    ...
    if (!mobile) error = "Missing mobile";
    else if (!name) error = "Missing name";
    else if (seenMobiles.has(mobile)) error = "Duplicate mobile in file";
    else if (existingMobiles.has(mobile)) error = "Customer already exists";

    if (!error) seenMobiles.add(mobile);
...
export function CustomerImportWizard() {
  ...
  const { data: presetsSetting, save: savePresets } = useAppSetting<ImportMappingPresetsSetting>("customerImportPresets", DEFAULT_IMPORT_MAPPING_PRESETS);
  ...
  function runValidation() {
    ...
    setResults(validateRows(table, mapping, existingCustomers || []));
  }
  ...
  async function runImport() {
    ...
    setImporting(false);
    if (failed) toast.error(`Imported ${validRows.length - failed} customers, ${failed} failed`);
    else toast.success(`Imported ${validRows.length} customers`);
    if (validRows.length - failed > 0) router.push("/crm");
  }
    ...
    <div className="space-y-5">
      {!table ? (
        <div className="rounded-xl border border-dashed p-8 text-center">
          ...
          <p className="mb-3 text-sm text-muted-foreground">Upload a CSV or TSV file — one row per customer.</p>
            ...
            <div className="rounded-xl border p-4">
              <div className="mb-3 flex items-center justify-between">
                ...
                <Button onClick={runImport} disabled={validCount === 0 || importing}>
                  {importing ? `Importing… (${importedCount}/${validCount})` : `Import ${validCount} customers`}
                </Button>
              </div>
