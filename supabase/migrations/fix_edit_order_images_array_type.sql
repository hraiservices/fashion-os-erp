-- Editing ANY order that has ever gone through edit_order() failed outright with
-- "COALESCE types jsonb and text[] cannot be matched" — orders.images is a real Postgres
-- text[] column (confirmed via information_schema.columns), but edit_order() has always
-- declared p_images as JSONB and assigned it straight into that column
-- (images = COALESCE(p_images, images)). Postgres's COALESCE requires both arguments to share
-- a type it can unify, and jsonb and text[] aren't one of the pairs it'll cast implicitly, so
-- this raised a parse-time type error on every single edit, not just some — this function
-- could never have successfully changed the images column since it was first written.
-- audios/videos/measurements/garments/history are all genuinely jsonb, so they're untouched.
--
-- Fix: convert the incoming JSONB array of URL strings to a real text[] before it ever reaches
-- COALESCE, via jsonb_array_elements_text() + ARRAY(). A NULL p_images (the "don't touch this
-- field" case every other param already relies on) skips straight to the existing column value.
