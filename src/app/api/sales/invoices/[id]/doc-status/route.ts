...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: invoice } = await supabase.from("sales_invoices").select("invoice_number").eq("id", id).maybeSingle();

  const { error } = await supabase.from("sales_invoices").update({ doc_status: parsed.data.docStatus }).eq("id", id);
