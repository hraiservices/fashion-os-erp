...
export async function POST(request: Request) {
  ...
  if (fd.id) {
    const { error } = await db.from("price_lists").update({ name: fd.name, notes: fd.notes }).eq("id", fd.id);
    if (error) return NextResponse.json({ error: error.message }, { status: 500 });
    return NextResponse.json({ id: fd.id });
  }

  const { data, error } = await db.from("price_lists").insert({ name: fd.name, notes: fd.notes, created_by: user.email }).select("id").single();
  if (error) return NextResponse.json({ error: error.message }, { status: 500 });
