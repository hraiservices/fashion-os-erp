...
import { useDeleteVendorPayment, useBulkDeleteVendorPayments } from "@/hooks/use-purchase-mutations";
...
export default function VendorPaymentsPage() {
  const { data: payments, isLoading: loadingPayments } = useAllVendorPayments();
  const { data: bills, isLoading: loadingBills } = usePurchaseBills();
  const { data: vendors, isLoading: loadingVendors } = useVendors();
  const { data: user } = useCurrentUser();
  const deletePayment = useDeleteVendorPayment();
  const bulkDelete = useBulkDeleteVendorPayments();
  const isLoading = loadingPayments || loadingBills || loadingVendors;
  ...
  const filtered = useMemo(() => {
    ...
    return (payments || []).filter((p) => {
      ...
      const vendorName = vendorNameById.get(p.vendorId) || "";
      return (bill?.billNumber || "").toLowerCase().includes(q) || vendorName.toLowerCase().includes(q);
    });
  }, [payments, search, billById, vendorNameById]);
  ...
  async function doDeleteOne(id: string) {
    ...
    setBusy(true);
    try {
      await deletePayment.mutateAsync({ id, amount: p.amount, billNumber: billById.get(p.billId)?.billNumber || "", userEmail: user?.email });
              ...
              {filtered.map((p) => {
                ...
                return (
                  <TableRow key={p.id}>
                    ...
                    <TableCell className="font-medium">{vendorNameById.get(p.vendorId) || "—"}</TableCell>
                    <TableCell>
                      {bill ? (
                        <Link href={`/purchases/bills/${bill.id}`} className="font-medium text-primary hover:underline">
                          {bill.billNumber}
                        </Link>
                      ) : (
                        "—"
      ...
      <AlertDialog open={!!confirmOne} onOpenChange={(v) => !v && setConfirmOne(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>This cannot be undone. The bill&apos;s balance will go back up by the payment amount.</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
      ...
      <AlertDialog open={confirmBulk} onOpenChange={setConfirmBulk}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>This cannot be undone. Each affected bill&apos;s balance will go back up by its payment amount.</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>Cancel</AlertDialogCancel>
