...
export async function DELETE(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { count } = await supabase.from("leave_requests").select("id", { count: "exact", head: true }).eq("leave_type_id", id);
  if (count && count > 0) {
    const { data, error } = await supabase.from("leave_types").update({ active: false }).eq("id", id).select("name").maybeSingle();
    if (error) return NextResponse.json({ error: error.message }, { status: 500 });
    await logAction(supabase, user.email, `Leave type deactivated (has requests, not deleted): ${data?.name}`);
    return NextResponse.json({ ok: true, deactivated: true });
  }

  const { data, error } = await supabase.from("leave_types").delete().eq("id", id).select("name").maybeSingle();
  if (error) return NextResponse.json({ error: error.message }, { status: 500 });
