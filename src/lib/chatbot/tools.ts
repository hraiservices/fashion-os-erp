 ...
 *
 * This replaces the old design where the model wrote the SQL itself (generateSql in gemini.ts).
 * That design's actual failure mode wasn't Gemini being a bad model — it's that "write correct
 * SQL against a schema you've only seen in a text description" is a much harder task than
 * "pick the right named tool and fill in its (few, typed) arguments." The model's job here is
 * reduced to tool selection + argument extraction, which is exactly what function-calling is
 * built for — it can no longer invent a wrong JOIN, wrong column, or wrong date filter, because
 * there is no query left for it to write.
 */
...
export const TOOL_DECLARATIONS: FunctionDeclaration[] = [
  {
    ...
    parameters: {
      type: Type.OBJECT,
      properties: {
        overdueOnly: { type: Type.BOOLEAN, description: "Only include orders past their delivery date (is_overdue). Default false." },
        tailor: { type: Type.STRING, description: "Filter to one tailor's name, if the question names one." },
      },
    },
  },
  {
    ...
    parameters: {
      type: Type.OBJECT,
      properties: {
        minDaysOverdue: { type: Type.NUMBER, description: "Only orders overdue by at least this many days." },
      },
    },
  },
  {
  ...
  },
  {
    ...
    parameters: {
      ...
      properties: {
        from: { type: Type.STRING, description: "Start date, YYYY-MM-DD (inclusive)." },
        to: { type: Type.STRING, description: "End date, YYYY-MM-DD (inclusive)." },
        method: { type: Type.STRING, description: "Filter to one payment method, e.g. 'cash', 'upi', 'card', if named." },
      },
      required: ["from", "to"],
    },
  },
  {
    ...
    parameters: {
      type: Type.OBJECT,
      properties: {
        status: { type: Type.STRING, description: "Filter to one status: 'unpaid', 'partial', or 'paid'." },
      },
    },
  },
  {
    ...
    parameters: {
      ...
      properties: {
        from: { type: Type.STRING, description: "Start date, YYYY-MM-DD (inclusive)." },
        to: { type: Type.STRING, description: "End date, YYYY-MM-DD (inclusive)." },
        category: { type: Type.STRING, description: "Filter to one expense category, if named." },
      },
      required: ["from", "to"],
    },
  },
  {
    ...
    parameters: {
      type: Type.OBJECT,
      properties: {
        itemType: { type: Type.STRING, description: "Filter to 'product' or 'raw_material', if the question is specific." },
      },
    },
  },
  {
  ...
  },
  {
    ...
    parameters: {
      type: Type.OBJECT,
      properties: {
        query: { type: Type.STRING, description: "Customer name (partial match ok) or mobile number." },
      },
      required: ["query"],
    },
  },
